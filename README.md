...
iss-number MCP — wraps StupidAPIs (keyless — no credential needed)
...
`tools/list` at `https://gateway.pipeworx.io/iss-number/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/iss_number_generate \
  -H 'Content-Type: application/json' \
  -d '{}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/iss_number_generate`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "iss-number": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-iss-number"]
    }
  }
}
```
